...
# Filter intronic annotations: keep only closest exon per transcript, add gene prefix
sub filter_intronic_annotation {
	my ($anno_str, $gene_name) = @_;
	return '.' unless $anno_str && $anno_str ne '.';
	
	my %tx_best;  # transcript -> (distance, exon_num, annotation)
	my @annos = split /;/, $anno_str;
	
	for my $anno (@annos) {
		# Parse: NM_021008.4:exon11:c.1504-2107->AAAAA
		if ($anno =~ /^(NM_\d+\.\d+):exon(\d+):c\.(\d+)([+-])(\d+)/) {
			my ($tx_id, $exon_num, $cdna_pos, $direction, $distance) = ($1, $2, $3, $4, $5);
			
			if (!exists $tx_best{$tx_id} || $distance < $tx_best{$tx_id}{distance}) {
				$tx_best{$tx_id} = {
					distance => $distance,
					annotation => $anno
				};
	...
	# Build filtered output with gene prefix
	my @filtered;
	for my $tx_id (sort keys %tx_best) {
		push @filtered, "$gene_name:" . $tx_best{$tx_id}{annotation};
	}
...
sub processArgument {
	...
	if (defined $intronhgvs) {
		...
		# Default: enable intronic cDNA annotation for all intronic variants
		$intronhgvs = 100000;
	}
...
sub geneOperation {
	...
	while (<FUNCTION>)
	{
		...
		my @typearray = (["Func", $function], ["Gene", $gene]);
		if ($xreffile) {
			for my $i (0 .. @xrefkey-1) {
		...
		# For intronic variants, move cDNA annotation to AAChange column
		# Filter to keep only closest exon per transcript, add gene name prefix
		if ($function eq 'intron' && $spliceanno && $spliceanno ne '.') {
			push @typearray, ["GeneDetail", "."];
			push @typearray, ["ExonicFunc", "."];
			# Parse and filter intronic annotations
			my $filtered_anno = filter_intronic_annotation($spliceanno, $gene);
			push @typearray, ["AAChange", $filtered_anno];
		} else {
			push @typearray, ["GeneDetail", $spliceanno];
		}
