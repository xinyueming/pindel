...
void BDData::loadBDFile(const std::string& filename)
{
   ...
   while (!bdFile.eof()) {
      ...
      if (firstChar == '#') {
         ...
         if (firstChrName == secondChrName && secondChrName != "" && (firstPos > secondPos ? firstPos - secondPos : secondPos - firstPos) < 500) {
            continue;
         }
...
{
  int distance = 1000;
  if ((first.PosA > first.PosA1 ? first.PosA - first.PosA1 : first.PosA1 - first.PosA) > distance) {
     return false;
  }
  if ((first.PosB > first.PosB1 ? first.PosB - first.PosB1 : first.PosB1 - first.PosB) > distance) {
     return false;
  }
  if ((second.PosA > second.PosA1 ? second.PosA - second.PosA1 : second.PosA1 - second.PosA) > distance) {
     return false;
  }
  if ((second.PosB > second.PosB1 ? second.PosB - second.PosB1 : second.PosB1 - second.PosB) > distance) {
     return false;
  }
  float cutoff = 0.9;
...
void ModifyRP(std::vector <RP_READ> & Reads_RP)
{
   ...
   for (unsigned first = 0; first < Reads_RP.size(); first++) {
      ...
      if (Reads_RP[first].ChrNameA == Reads_RP[first].ChrNameB && (Reads_RP[first].PosA > Reads_RP[first].PosB ? Reads_RP[first].PosA - Reads_RP[first].PosB : Reads_RP[first].PosB - Reads_RP[first].PosA) < 500) {
